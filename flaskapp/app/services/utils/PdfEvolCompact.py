...
class PdfEvolCompact(ReportLabCanvasUtils):
    ...
    def create_professional_info_text(self, professional:dict, date: str, nullable:bool=True):
        ...
        self.validate_func_args(function_to_verify=self.create_professional_info_text, variables_to_verify={'professional':professional, 'nullable':nullable, 'date':date})
        ...
        if not nullable:
            ...
            for camp in [name, document]:
                if camp == None:
                    raise Exception('Algum campo do profissional está faltando, o profissional precisa do nome, e documento no formato "54321/BA"')
        ...
        prof_info = f"{str(name).strip()} {document_category} {str(document)}" + ' Criado em: ' + str_date
    ...
    def add_prescription(self, prescription:list) -> None:
        ...
        for presc in prescription:
            ...
            if count != 1:
                ...
                self.add_rectangle(pos=(17, rect_y_pos), height=DEFAULT_RECT_HEIGHT, width=DEFAULT_RECT_WIDTH, fill=1, color=(0.5215, 0.5215, 0.5215 ))
            yposition = self.add_morelines_text(text=current_text, initial_pos=(18, yposition), decrease_ypos=DEFAULT_DECREASE_Y_POS, field_name=f'{count} Prescription', len_max=4032, char_per_lines=CHAR_PER_LINES, return_ypos=True)
            if yposition <= 110:
    ...
    def get_measures_list(self, date_object:datetime, measure:dict):
        ...
        measures_list = [created_at, spO2, pain, blood_pressure, cardiac_frequency, respiratory_frequency, celcius_axillary_temperature, glucose, fetal_cardiac_frequency]
    ...
    def add_measures(self, measures:list) -> None:
        ...
        try:
            ...
            for m in measures:
                ...
                measures_list = self.get_measures_list(date_object=date_object, measure=m)
                ...
                for value in measures_list:
                    ...
                    self.add_oneline_text(text=str(value), pos=(x_pos, all_list_ypos.pop(0)), field_name=f'{global_count} Measure', len_max=10, len_min=1, centralized=True)
