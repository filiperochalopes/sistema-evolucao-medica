...
def func_generate_pdf_apac(requesting_establishment:dict, patient:dict, main_procedure:dict,patient_responsible_name:str=None, patient_color:str=None, document_chart_number:str=None, procedure_justification_description:str=None, procedure_justification_main_cid_10:str=None, procedure_justification_sec_cid_10:str=None, procedure_justification_associated_cause_cid_10:str=None, procedure_justification_observations:str=None,requesting_professional_document:dict=None, requesting_professional_name:str=None, establishment_exec:dict=None, contacts_phonenumbers:list=None,solicitation_date:datetime.datetime=None, professional_authorization_name:str=None, emission_org_code:str=None, authorization_professional_document:dict=None, authorization_date:datetime.datetime=None, signature_date:datetime.datetime=None, validity_period_start:datetime.datetime=None, validity_period_end:datetime.datetime=None, secondaries_procedures:list=None) -> str:
    ...
    try:
        ...
        # not null data
        try:
            pdf.add_cns(cns=patient['cns'], pos=(36, 678), field_name='Patient CNS', interval='  ')
            pdf.add_procedure(procedure=main_procedure, code_pos=(36,542), name_pos=(220, 542), quant_pos=(508, 542), field_name='Main Procedure')

            pdf.set_font('Roboto-Mono', 9)
            pdf.add_oneline_text(text=requesting_establishment['name'], pos=(36, 742), field_name='Establishment Solict Name', len_max=77, len_min=7)
            pdf.add_oneline_text(text=patient['name'], pos=(36, 702), field_name='Patient Name', len_max=67, len_min=7)
            pdf.add_sex_square(sex=patient['sex'], pos_male=(423, 699), pos_fem=(456, 699), field_name='Patient Sex', square_size=(9,9))
            pdf.add_datetime(date=patient['birthdate'], pos=(315, 678), field_name='Patient Birthday', hours=False, interval='  ', formated=False)
            pdf.add_oneline_text(text=patient['address']['city'], pos=(36, 584), field_name='Patient Adress City', len_max=58, len_min=3)
            pdf.add_oneline_text(text=requesting_establishment['cnes'], pos=(468, 742), field_name='Establishment Solict CNES', len_max=7, len_min=7)
        ...
        try:
            ...
            if establishment_exec is None:
                # Create empty dict
                establishment_exec = {'':''}
            pdf.add_oneline_text(text=establishment_exec.get('cnes'), pos=(450, 28), field_name='Establishment Exec CNES', len_max=7, len_min=7,interval=' ', nullable=True)
            pdf.set_font('Roboto-Mono', 9)
            ...
            pdf.add_oneline_text(text=str(procedure_justification_description).upper(), pos=(36, 344), field_name='Procedure Justification Description', len_max=55, len_min=4, nullable=True)
            ...
            pdf.add_oneline_text(text=patient_color, pos=(404, 678), field_name='Patient Color', len_max=10, len_min=4, nullable=True)
            pdf.add_oneline_text(text=patient.get('ethnicity'), pos=(470, 678), field_name='Patient Ehinicity', len_max=17, len_min=4, nullable=True)
            pdf.add_oneline_text(text=document_chart_number, pos=(483, 702), field_name='Document Chart Number', len_max=14, len_min=1, nullable=True)
            pdf.add_oneline_text(text=procedure_justification_main_cid_10, pos=(352, 344), field_name='Procedure Justification main CID10', len_max=4, len_min=3, nullable=True)
            pdf.add_oneline_text(text=procedure_justification_sec_cid_10, pos=(420, 344), field_name='Procedure Justification secondary CID10', len_max=4, len_min=3, nullable=True)
            pdf.add_oneline_text(text=procedure_justification_associated_cause_cid_10, pos=(505, 344), field_name='Procedure Justification Associated Causes CID10', len_max=4, len_min=3, nullable=True)
            pdf.add_oneline_text(text=requesting_professional_name, pos=(36, 204), field_name='Profissional Solicitor Name', len_max=48, len_min=5, nullable=True)
            pdf.add_oneline_text(text=professional_authorization_name, pos=(36, 136), field_name='Profissional Authorizator Name', len_max=46, len_min=5, nullable=True)
            pdf.add_oneline_text(text=emission_org_code, pos=(290, 136), field_name='Emission Org Code', len_max=16, len_min=2, nullable=True)
            pdf.add_datetime(date=solicitation_date, pos=(310, 204), field_name='Solicitation Datetime', hours=False, interval='  ', formated=False, nullable=True)
            pdf.add_datetime(date=authorization_date, pos=(36, 68), field_name='Authorization Datetime', hours=False,formated=True, nullable=True)
            pdf.add_datetime(date=signature_date, pos=(154, 68), field_name='Signature Datetime', hours=False, interval='  ', formated=False, nullable=True)
