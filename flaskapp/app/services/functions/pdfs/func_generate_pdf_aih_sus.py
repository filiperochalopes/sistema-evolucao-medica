...
def func_generate_pdf_aih_sus(requesting_establishment:dict, patient:dict, main_clinical_signs_symptoms:str, conditions_justify_hospitalization:str, initial_diagnosis:str, principal_cid_10:str, requesting_professional_document:dict, requesting_professional_name:str, request_date:datetime.datetime, establishment_exec:dict=None,requested_procedure:str=None, procedure_code:str=None, clinic:str=None, internation_carater:str=None, emission_org_code:str=None, hospitalization_authorization_number:str=None, authorization_professional_document:dict=None, authorization_date:datetime.datetime=None, professional_authorization_name:str=None, exam_results:str=None, chart_number:str=None, patient_responsible_name:str=None, contacts_phonenumbers:list=None, secondary_cid_10:str=None, cid_10_associated_causes:str=None, acident_type:str=None, insurance_company_cnpj:str=None, insurance_company_ticket_number:str=None, insurance_company_series:str=None,company_cnpj:str=None, company_cnae:int=None, company_cbor:int=None, pension_status:str=None) -> str:
    ...
    try:
        ...
        try:     
            ...
            pdf.add_cns(cns=patient['cns'], pos=(28, 658), field_name='Patient CNS', interval='  ')
            pdf.add_oneline_text(text=patient['address']['zip_code'], pos=(482, 566), field_name='Patient Adress CEP', len_max=8, len_min=8, interval=' ')
            
            pdf.set_font('Roboto-Mono', 9)
            pdf.add_datetime(date=patient['birthdate'], pos=(312, 658), field_name='Patient Birthday', hours=False, interval='  ', formated=False)
            pdf.add_sex_square(sex=patient['sex'], pos_male=(415, 657), pos_fem=(468, 657), field_name='Patient Sex', square_size=(8,9))
            pdf.add_oneline_text(text=patient['mother_name'], pos=(25, 636), field_name='Patient Mother Name', len_max=70, len_min=7)
            pdf.add_oneline_text(text=patient['address']['street'], pos=(25, 593), field_name='Patient Adress', len_max=101, len_min=7)
            pdf.add_oneline_text(text=patient['address']['city'], pos=(25, 566), field_name='Patient Adress City', len_max=58, len_min=3)
            pdf.add_oneline_text(text=patient['address']['ibge_city_code'], pos=(388, 566), field_name='Patient Adress City IBGE code', len_max=7, len_min=7)
            pdf.add_UF(uf=patient['address']['uf'], pos=(450, 566), field_name='Patient Adress UF', interval='  ')
            pdf.add_morelines_text(text=main_clinical_signs_symptoms, initial_pos=(25, 530), decrease_ypos= 10, field_name='Main Clinical Signs Symptoms', len_max=1009, char_per_lines=101, len_min=5)
        ...
        try:
            pdf.add_oneline_text(text=requested_procedure, pos=(25, 269), field_name='Procedure Solicited', len_max=65, len_min=6, nullable=True)
            pdf.add_oneline_text(text=procedure_code, pos=(404, 269), field_name='Procedure Code', len_max=10, len_min=10, interval='  ', nullable=True)
            pdf.add_oneline_text(text=clinic, pos=(25, 246), field_name='Clinic', len_max=18, len_min=6, nullable=True)
            pdf.add_oneline_text(text=internation_carater, pos=(128, 246), field_name='Internation Caracter', len_max=19, len_min=6, nullable=True)
            if establishment_exec is not None:
                pdf.add_oneline_text(text=establishment_exec.get('name'), pos=(25, 726), field_name='Establishment Exec Name', len_max=82, len_min=8, nullable=True)
                pdf.add_oneline_text(text=establishment_exec.get('cnes'), pos=(470, 726), field_name='Establishment Exec CNES', len_max=7, len_min=7,interval='  ', nullable=True)
            pdf.add_morelines_text(text=exam_results, initial_pos=(25, 362), decrease_ypos= 10, field_name='Exam Results', len_max=403, char_per_lines=101, len_min=5, nullable=True)            
            pdf.add_oneline_text(text=chart_number, pos=(466, 683), field_name='Chart Number', len_max=20, len_min=1,nullable=True)            
            pdf.add_oneline_text(text=patient.get('ethnicity'), pos=(510, 658), field_name='Patient Ehinicity', len_max=11, len_min=4, nullable=True)
            pdf.add_oneline_text(text=patient_responsible_name, pos=(25, 612), field_name='Patient Responsible Name', len_max=70, len_min=7, nullable=True)        
            pdf.add_contact_phonenumbers(phone_numbers=contacts_phonenumbers, pos=(415, 631), interval='  ', y_decrease=23, nullable=True)
            pdf.add_oneline_text(text=secondary_cid_10, pos=(406, 314), field_name='Secondary Cid10', len_max=4, len_min=3, nullable=True)
            pdf.add_oneline_text(text=cid_10_associated_causes, pos=(512, 314), field_name='Associated causes Cid10', len_max=4, len_min=3, nullable=True)
            pdf.add_markable_square(option=acident_type, valid_options=['TRAFFIC', 'WORK', 'WORK_PATH'], options_positions=((38,184),(38,170), (38,156)), square_size=(9,9), field_name='Acident Type', nullable=True)

            pdf.set_font('Roboto-Mono', 16)       
            pdf.add_oneline_text(text=hospitalization_authorization_number, pos=(480, 66), field_name='Hospitalization authorization Number', len_max=18, len_min=1, centralized=True, nullable=True)
            
            # Data that change Font Size
            pdf.set_font('Roboto-Mono', 10)
            pdf.add_cnpj(cnpj=insurance_company_cnpj, pos=(168,183), field_name='Insurance Company CNPJ', nullable=True, interval='  ')           
            pdf.add_cnpj(cnpj=company_cnpj, pos=(168,156), field_name='Company CNPJ', nullable=True, interval='  ')
            ...
            pdf.add_oneline_text(text=emission_org_code, pos=(292, 93), field_name='Emission Organization Code', len_max=17, len_min=2, nullable=True)
            pdf.add_oneline_text(text=professional_authorization_name, pos=(25, 93), field_name='Professional Authorizator Name', len_max=48, len_min=8, nullable=True)
            pdf.add_document_cns_cpf_rg(document=authorization_professional_document, pos_square_cpf=(95, 66), pos_square_cns=(41,66), pos_cns=(146, 66), pos_cpf=(146, 66),field_name='Professional Authorizator Document', interval='  ', nullable=True)
            pdf.add_datetime(date=authorization_date, pos=(30, 30), field_name='Authorization Datetime', hours=False, interval='  ', formated=False, nullable=True)
