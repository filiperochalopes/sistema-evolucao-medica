...
def cid10(*_, query:str='', per_page:int=25, page:int=1):   
    ...
    return db.session.query(Cid10).filter(or_(Cid10.description.ilike(f"%{query}%"),Cid10.code.like(f"%{query}%"))).order_by(Cid10.code).paginate(page=page, per_page=per_page)