...
from app.tests.pdfs.request_queries_examples import ficha_internamento_request_strings
...
def test_all_test_queries(client):
    for request_string in ficha_internamento_request_strings:
        assert make_request(client, request_string) == True


def test_create_pdf_file_from_queries(client):
    PDF_START_STRING = 'ficha_internamento'
